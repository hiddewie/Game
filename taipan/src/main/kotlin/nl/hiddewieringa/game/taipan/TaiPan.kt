        ...
        listOf(
            ...
            GameDecision(outOfCardOrder.size == 3 || (outOfCardOrder.size >= 2 && outOfCardOrder[0].team == outOfCardOrder[1].team)) {
                ...
                if (outOfCardOrder[0].team == outOfCardOrder[1].team) { // double game
                    scores.computeIfPresent(outOfCardOrder[0].team) { _, s -> s + 200 }
                } else {
                    scores.computeIfPresent(outOfCardOrder[0].team) { _, s -> s + roundCards.getValue(outOfCardOrder[0]).sumBy { it.points } }
                    scores.computeIfPresent(outOfCardOrder[1].team) { _, s -> s + roundCards.getValue(outOfCardOrder[1]).sumBy { it.points } }
                    scores.computeIfPresent(outOfCardOrder[2].team) { _, s -> s + roundCards.getValue(outOfCardOrder[2]).sumBy { it.points } }

                    val lastPlayer = TwoTeamPlayerId.values().filterNot { outOfCardOrder.contains(it) }[0]
                    // Tricks of last player go to first player.
                    scores.computeIfPresent(outOfCardOrder[0].team) { _, s -> s + roundCards.getValue(lastPlayer).sumBy { it.points } }
                    // Hand of last player goes to opposite team
                    scores.computeIfPresent(outOfCardOrder[0].team) { _, s -> s + playerCards.getValue(lastPlayer).sumBy { it.points } }
                }
                taiPannedPlayers.forEach { (playerId, taiPan) ->
                    scores.computeIfPresent(playerId.team) { _, s ->
...
data class TaiPanDragonPass(val trick: TaiPanPlayTrick) : TaiPanState(), IntermediateGameState<TwoTeamPlayerId, TaiPanPlayerActions, TaiPanEvent, TaiPanState> {
